...
public class Trader {
    ...
    public static void deposit(int n, Player p, UUID uuid) {
        ...
        StonkExchange.logs.log("(DEPOSIT) " + Bukkit.getOfflinePlayer(uuid).getName() + " -> " + Math.round((b.getSolde() - init)*1000f)/1000f);
    }
    ...
    public static void withdraw(int n, Player p,UUID uuid) {
        ...
        StonkExchange.logs.log("(WITHDRAW) " + Bukkit.getOfflinePlayer(uuid).getName() + " -> " + Math.round((init - b.getSolde())*1000f)/1000f);
    }
}
